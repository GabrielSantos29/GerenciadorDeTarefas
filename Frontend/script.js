...
async function carregarTarefas() {
    try {
        ...
        tarefas.forEach(tarefa => {
            ...
            `;
            // Se o modoRemoção estiver ativo, adicionar os botoes de remoção
            if(modoRemocao){
                conteudo += `<button class="btn-remover-tarefa" data-id="${tarefa.id}">❌ Remover</button>`;
            }
            // Se o modoEdição estiver ativo, adicionar os botoes de edição
            if(modoEdicao){
        ...
        if(modoEdicao){
            const botoesEdicao = document.getElementsByClassName("btn-editar-tarefa");
